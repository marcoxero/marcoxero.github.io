...
function InitPaaS(filter, rawData, boardId, miroCreateShape) {
    ...
    for(d in domains){
        ...
        miroCreateShape(boardId, domain.name, domain.x + domain.width/2, domain.y, 100, 70, 10, "center", "middle", "hexagon", function (domainData){            
            var x = domain.x;
            var y = domain.y + 110;
            for(g in domain.children){
                const capabilityGroup = domain.children[g];
                capabilityGroup.x = x;
                capabilityGroup.y = y;
                capabilityGroup.width = 120;
                x += capabilityGroup.width + 10;
        
                miroCreateShape(boardId, capabilityGroup.name, capabilityGroup.x, capabilityGroup.y, 110, 100, 10, "center", "middle", "rectangle", function (domainData){
                    var x = capabilityGroup.x;
                    var y = capabilityGroup.y + 80;
                    for(c in capabilityGroup.children){
                        const capability = capabilityGroup.children[c];
                        capability.x = x;
                        capability.y = y;
                        ...
                        miroCreateShape(boardId, capability.name, capability.x, capability.y + capability.height/2, capability.width, capability.height, 10, "center", "top", "rectangle", function (domainData){
                            var x = capability.x;
                            ...
                            for(i in capability.children){
                                ...
                                miroCreateShape(boardId, item.name, item.x, item.y + 50, 100, 100, 10, "center", "middle", "rectangle", null);
                            }                        
                        });
                    }
                });
